...
class Part_Attention2(nn.Module):
    def __init__(self):
        super(Part_Attention2, self).__init__()
...
class Encoder(nn.Module):
    def __init__(self, config, vis):
        ...
        if config.feature_fusion:
            ...
            self.part_select2 = Part_Attention2()
    ...
    def forward(self, hidden_states):
        ...
        for layer_block in self.layer:
            ...
            if self.feature_fusion:
                ...
                #if c%2!=0 and c!=0:
                if c==0:
                    selected_num, selected_inx = self.part_select(attn_weights)
                    B = selected_inx.shape[0]
                    for i in range(B):
                        tokens[i].extend(hidden_states[i, selected_inx[i,:self.num_token]])
                else:
                    selected_num, selected_inx = self.part_select2(attn_weights[c], attn_weights[c-1])
                    B = selected_inx.shape[0]
                    for i in range(B):
                        tokens[i].extend(hidden_states[i, selected_inx[i,:self.num_token]])
                #if c < 5 and c!=0:
