...
from attentions import attention_crop_drop
from attentions import mask2bbox
...
def valid(args, model, writer, test_loader, global_step):
    ...
    for step, batch in enumerate(epoch_iterator):
        ...
        with torch.no_grad():
            #logits = model(x)[0]
            output1, attns = model(x)
            refined_input = mask2bbox(attns, x)
            output2, _ = model(refined_input)
            logits = (F.softmax(output1, dim=-1)+F.softmax(output2, dim=-1))/2
...
def train(args, model):
    ...
    feature_center = torch.zeros(200, 200).to(args.device)
    ...
    while True:
        ...
        for step, batch in enumerate(epoch_iterator):
            batch = tuple(t.to(args.device) for t in batch)
            x, y = batch
            loss1, logits, attns = model(x, y)
            #loss = loss.mean() # for contrastive learning
            img_crop, img_drop = attention_crop_drop(attns, x)
            
            loss2, _, _  = model(img_drop, y)
            loss3, _, _  = model(img_crop, y)
...
def main():
    ...
    parser.add_argument("--dataset", choices=["cifar10", "cifar100", "soyloc","cotton", "CUB", "dog","car","air"], default="CUB",
    ...
    parser.add_argument('--data_root', type=str, default="/home/pengtl/jackhu/")
